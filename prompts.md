...
我希望以后再遇到这两个UI组件的时候，不要再造轮子了。
一个是音量和被压制的音量的这个界面，另一个是频谱的界面。
你觉得要怎么样抽象出来？
想单独听被削减的声音这种定制的需求就不需要抽象出来了

---

如果按照这样设计，Rust侧FFT的参数也暴露出去。然后JS侧的一些颜色参数也需要暴露出去。然后数据协议上，每一个插件各自定义，不要定死，因为这部分代码也比较好写。帮我解释一下整个架构

---

rust-auv2-docs/agent-workflow.md
这是我的LLM辅助构建插件工作流，现在我有一个压缩器DSP的实现位于rust-dsp-crates/compressor/zlcompressor-rs
它是根据ZLCompressor/这个目录下的C++ 版本改写的,他即将被以上述工作流的方式集成到我们将要构建的一个插件中.
现在我需要你帮我审计一下，这个Rust的移植是否和C++ 的是一致的?
可能缺少的部分是F64的处理，但是我们目前只需要用于32位的音频，因此这个可以放过，其他的都需要进行审计

---
我们需要：
mid/side + l/r
测试音频使用audio/chichi.mp3
以及修复所有其他的bug
