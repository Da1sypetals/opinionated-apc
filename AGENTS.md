...
除非用户显式要求，否则不允许擅自用Git回滚任何代码

所有shell command都必须让输出真实打印出来，不允许在后面接grep, rg, tail, head；默认不允许重定向。如果太长了你再去查询。

给我谦虚一点，每次改完之后，不允许自吹自擂，你需要自己验证或者问我这个修改是否有效，而不是直接告诉我这个修改就有效了

不允许使用视觉能力，因为你的视觉的精准度是极低的。