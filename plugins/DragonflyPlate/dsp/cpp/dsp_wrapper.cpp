...
void* dragonfly_plate_create(int32_t sample_rate)
{
    return new DragonflyReverbDSP(static_cast<double>(sample_rate));
}

void dragonfly_plate_destroy(void* engine)
{
    delete static_cast<DragonflyReverbDSP*>(engine);
}

void dragonfly_plate_set_sample_rate(void* engine, int32_t sample_rate)
{
...
void dragonfly_plate_set_parameter(void* engine, uint32_t index, float value)
{
    static_cast<DragonflyReverbDSP*>(engine)->setParameterValue(index, value);
}

float dragonfly_plate_get_parameter(void* engine, uint32_t index)
{
    return static_cast<DragonflyReverbDSP*>(engine)->getParameterValue(index);
}

void dragonfly_plate_process(
    void* engine,
    const float* in_l,
    const float* in_r,
    float* out_l,
...
void dragonfly_plate_clear_buffers(void* engine)
{
